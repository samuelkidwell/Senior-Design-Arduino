...
const int ledPin05 = 11;          // LED pin for 0.5 kg threshold (blue LED)
const int ledPin1 = 12;           // LED pin for 1 kg threshold (green LED)

// HX711 initialization
HX711 scale;

// Use the calibration factor obtained during calibration
float calibration_factor = 11045.4795;

// Set threshold for maximum weight
const float maxThresholdKg = 2.0; // Set threshold for maximum weight
...
void loop() {
    ...
    if (scale.is_ready()) {
        ...
        float weightKg = abs(weight / 1.9759);    // Conversion for correct weight, also keeps values positive
        ...
        // Check if the weight exceeds the maximum threshold
        if (weightKg >= maxThresholdKg) {
            // Weight is above 1 kg, turn on the 1 kg LED
            digitalWrite(ledPin1, HIGH);    // LED for 1 kg
            analogWrite(ledPin05, 0);       // Turn off the blue LED

            // Activate the speaker and motor
            if (!motorRunning) {
            ...
            // If weight is below threshold, start the no weight timer
            if (!noWeightDetected) {
